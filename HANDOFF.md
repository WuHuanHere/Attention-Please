...
| **摄像头让出** | ✅ **已真机验证**(2026-09-18 20:17 用 OBS):`📷 已让出摄像头(前台是「OBS」)` → 1 秒后释放 → OBS 关掉后 4 秒收回 → 2 秒后判定恢复(4.5 Hz / 人脸 90%)。事件库里 `camera_yield`/`camera_yield_end` 齐全 | 下次用**会议软件**(腾讯会议/钉钉)再验一次; 认不出来的软件用托盘「让出摄像头(30 分钟)」并把窗口标题关键词加进 `[camera_yield] apps` |
