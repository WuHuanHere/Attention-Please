...
54 个 .py 文件、8785 行(源码 3886 行 / 19 个,测试 3462 行 / 27 个,脚本 1437 行 / 10 个;
另有 `probe_out/` 23 个文件 2017 行,不进版本库)。**313 项单测**(7 项需 `AP_UI_TESTS=1`)。
...
- **`probe_out/analyze_log.py` 的"可疑分集"标记不考虑冷却**:一段 64 秒、最高 L0 的分集
  ...
  对照 `nudge` 事件时间再下结论。
- ⚠️ **分集被"程序被杀"截断时不会写 `episode_end`** → 那次分心**既不计时长也不计次数**,
  但 `nudge` 还在。于是日报会出现"提醒 2 次 / 分心 0 分钟"这种自相矛盾的行
  (2026-09-20 那份冻结的报告里就有)。按"漏报必须可见"的铁律这个洞要补:
  要么启动时收尾上次未结束的分集, 要么在日报里把"没有收尾的分集"单列出来。
