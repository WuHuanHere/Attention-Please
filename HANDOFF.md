...
| 发呆信号(daze) | ❌ 已实现但**不建议开**,而且**目前是"哑"的**(眨眼闸门永不成立,见 §9) |
...
| 单测 | ✅ **285 项全绿**(7 项 UI 测试默认跳过) |
| 版本控制 | ✅ **已建 git 仓库**(2026-09-18),`main` 分支,58 个文件已提交;`.venv`/模型/`data/`/`captures/`/`calibration.json`/`probe_out/` 已忽略 |

### 代码规模

54 个 .py 文件、8124 行(源码 3634 行 / 19 个,测试 3053 行 / 25 个,脚本 1437 行 / 10 个;
另有 `probe_out/` 15 个文件 1346 行,不进版本库)。
...
### M4(进行中):Pose 弱证据通道**已建好并接线**;**发呆的处置还没定**
...
- **新增 `tests/test_baseline.py`(20 项)/ `test_pose_head.py`(32 项)/
  `test_pose_evidence.py`(19 项)= 71 项新测试**(214 → 285)。

#### 还没做(下一步)

| 项 | 现状 | 怎么做 |
|---|---|---|
| **真人标定 `pose_head_min_margin`** | 现在的默认 `0.08` **只由 1 个真人低头样本支撑** | 摄像头空闲时跑 `probe_out/poselabel.py`(约 2 分钟;屏幕上有姿势提示 + 倒计时,并直接给出"分得开/分不开"的结论和建议阈值)。**摄像头被监控实例独占, 需要先暂停 2 分钟让它释放** |
| **发呆的处置:修 or 砍** | **发呆目前其实是"哑"的** —— 眨眼闸门用校准基线 3.0/分,`blink_rate < 1.5` 几乎永不成立,所以它既不误报也不生效 | 等标定数据。若 Pose 低头判据"分不开",按 §9 预案**砍掉发呆**,只留 screen + phone + Pose 记录通道 |
| **M4-E 眨眼基线自适应** | **故意没做** | 眨眼只被发呆用到;发呆若砍了它就是白做。发呆保留再做 |
| **重启托盘** | 正在跑的实例是 **13:30 的旧代码**,没有 pose 通道 | 让用户重启 `pythonw scripts\run_tray.py` |
...
| **Pose 通道本身** | 代码就绪、单测通过, **从未在真实机器上跑过**(实例是旧代码) | 重启托盘后看日志里的 `Pose:低头/转头` 与日报的「看不清」拆分 |
