...
| 项 | 现状 | 怎么做 |
|---|---|---|
| **发呆的处置:修 or 砍** | 标定后**证据已经足够下结论**(见下) | 需要用户拍板(建议**砍掉**,理由见下) |
| **M4-E 眨眼基线自适应** | **故意没做** | 眨眼只被发呆用到;发呆若砍了它就是白做 |
| ~~重启托盘~~ | ✅ 已于 18:56 重启,当前实例含 pose 通道 + 全部修复 | —— |

**关于发呆的建议(等用户拍板)**:三条独立证据都指向"砍掉" ——
...
信号没有可行的证据链。建议按预案砍掉,只留 screen + phone + pose(记录)。
...
| **Pose 通道本身** | ✅ **已真机验证**(18:26 那次):抓到了 `EpisodeStart pose record_only=True` / `EpisodeEnd pose duration=32.9`, **一声没响**;18:56 已重启带全部修复的版本 | 看今晚 21:30 日报的「看不清」拆分(9/18 全天有 63.9 分钟「看不清」= 监控时长的 17%) |
