...
- **各时段专注情况(按你自己的作息表)**:日报会把一天按 `[schedule]` 切成一个个时段,
  逐段给出 计划 / 实际监控 / 有效专注 / 专注率 / 分心 / 看不清 / 离开,再给一行合计和
  一行"时间表外"(手动"现在开始学习"、提前开机、拖堂都算这里)。这样"上午两小时全神贯注、
  下午三小时全废"不会被平均成一个看不出问题的数字。三条口径:
  - 专注率的分母是**实际监控时长**(和标题一致),不是计划时长;
  - **还没到的时段写「未到」,已经过去却一分钟都没监控到的写「无监控数据」**,都不写 0%
    —— 中午打开日报时,下午那个 0% 是假的;
  - 不足一分钟的写"20 秒",不写"0 分钟";各段加起来和合计差够 1 分钟时会自己写一行 ⚠️。

- **在托盘里点「打开今日日报」看到的是渲染好的预览**,不是一堆 `| a | b |` 记号:
  标题/引用/列表/加粗各有样式,表格按列对齐(中英混排也不会错位),隔行有底色;
  窗口右下角有「看纯文本」可以切回原始 markdown。磁盘上的 `.md` 文件本身不变,
  还是给编辑器和 git 看的。
- 如果程序**在分心过程中被强行关掉/崩溃/蓝屏**,那段分心没有结尾,时长也就无从得知 ——
  日报会写「有 N 段分心没有收尾」并说明它**没有**计入分心时长(宁可缺,不可假),
  托盘启动时也会当场提醒你一句。正常从托盘「退出」则会自动把这段分心结掉。
  离开座位同理(「离开座位 + N 段没等到收尾」)。
- **背单词时段**(`allow_phone = true`)用手机只记录不报警,**也不算分心** ——
  日报会单列一行「其中「允许用手机」时段」,让你看得见但不会被扣专注。
- 你在提醒窗上点「判定错了」的那一段,会**从分心时长里剔除**(单列成一行),
  不会再一边说"你标记了误判"一边照样扣你的专注。
- 提醒窗口自己报错时会写进**数据可信度**那一段(那几次你可能只听到声音、没看到弹窗)。
- **白名单优先是刻意设计**: 你用知乎/B站查考研资料、看题目解析,所以
  「考研数学基础班 - 知乎」这种会算**专注** —— 学习词(`数学/英语/考研/真题`)
  故意赢过站点名(`知乎/哔哩哔哩/微博`)。日报会中性披露"有多少次是这样判的",
  次数多说明黑名单基本不起作用了,那时该收窄 `[whitelist]` 里的通用词。
